// leetcode-420-字符串最小修改次数.cpp : 定义控制台应用程序的入口点。
//
		...
		*/
		for (int i = 0, numLetters, dec; i < 3; i++) {
			for (auto it = lenCnts[i].begin(); it != lenCnts[i].end(); it++) {
				if (i < 2) {
					numLetters = i + 1, dec = min(it->second, (deleteTarget - toDelete) / numLetters);//the num of pair i we need to delete 
		...
		//all the remaining repetition mod 3 is 2
		//delete 3 elements for each repetition and the number of deletion decrease the replacement by 1
